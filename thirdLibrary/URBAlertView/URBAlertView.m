...
#define kURBAlertButtonOffset 44.0
...
- (void)initialize {
	...
	_strokeColor = [UIColor colorWithWhite:1 alpha:1.000];
	_strokeWidth = 0.0;
	_cornerRadius = 6.0;
	...
	_buttonBackgroundColor = [UIColor colorWithWhite:1 alpha:1.0];
	[self updateBackgroundGradient];
		
	self.animationType = URBAlertAnimationFade;
...
- (void)setTitleTextAttributes:(NSDictionary *)textAttributes {
	...
	UIColor *textShadowColor = ((NSShadow *)[textAttributes objectForKey:NSShadowAttributeName]).shadowColor;
	NSValue *shadowOffsetValue = [NSValue valueWithCGSize:((NSShadow *)[textAttributes objectForKey:NSShadowAttributeName]).shadowOffset];
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
	if (backgroundColor != _backgroundColor) {
		...
		[self updateBackgroundGradient];
		self.lightKeylineColor = [self.backgroundColor adjustBrightness:0.2];
...
- (void)setLeftImage:(NSString*)imageName {
    self.leftImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
    self.leftImageView.frame = CGRectMake(0, self.frame.size.height - 44 - 125, 68, 125);
...
- (void)layoutSubviews {
    ...
    self.leftImageView.frame = CGRectMake(23, self.frame.size.height - 60 - 125, 68, 125);
}
...
- (void)layoutComponents {
	...
	if (self.title.length > 0) {
		if ([self.title respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
            ...
            CGSize titleSize = [self.title boundingRectWithSize:CGSizeMake(CGRectGetWidth(textFrame), MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
    ...
    float frameWidth = CGRectGetWidth(textFrame);
    
    if (self.leftImageView == nil) {
        
    } else {
        frameWidth = CGRectGetWidth(textFrame) - 71;
    }
	...
	if (self.message.length > 0) {
		if ([self.title respondsToSelector:@selector(boundingRectWithSize:options:attributes:context:)]) {
			CGRect messageBounds = [self.message boundingRectWithSize:CGSizeMake(frameWidth, MAXFLOAT)
															  options:(NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading)
            ...
            NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc]init];
            paragraphStyle.lineBreakMode = NSLineBreakByWordWrapping;
            NSDictionary *attributes = @{NSFontAttributeName:self.messageFont, NSParagraphStyleAttributeName:paragraphStyle.copy};
            CGSize messageSize = [self.message boundingRectWithSize:CGSizeMake(frameWidth, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:attributes context:nil].size;
    ...
    if (self.leftImageView == nil) {
        layout.messageRect = CGRectMake(CGRectGetMinX(textFrame), minY, frameWidth, messageHeight);
    } else {
        layout.messageRect = CGRectMake(CGRectGetMinX(textFrame) + 70, minY, frameWidth, messageHeight);
    }
    ...
    if (self.shouldHorizontal) {
        dialogFrame.origin.x = (CGRectGetHeight(window.bounds) - CGRectGetWidth(dialogFrame)) / 2.0;
        dialogFrame.origin.y = (CGRectGetWidth(window.bounds) - CGRectGetHeight(dialogFrame)) / 2.0;
    }else{
...
- (void)drawRect:(CGRect)rect {
    ...
    if ([self.buttons count] == 2) {
        UIBezierPath *keySeplinePath = [UIBezierPath bezierPath];
        [keySeplinePath moveToPoint:CGPointMake(CGRectGetMinX(backgroundBaseRect) + CGRectGetWidth(backgroundBaseRect) * 0.5f, 0)];
        
        CGPoint p = CGPointMake(CGRectGetMinX(backgroundBaseRect) + CGRectGetWidth(backgroundBaseRect) * 0.5f, layout.buttonRegionRect.size.height+1);
        
        [keySeplinePath addLineToPoint:p];
        keySeplinePath.lineWidth = 0.1;
        [keySeplinePath stroke];
    }
    ...
    [self drawText:self.title textAlignment:NSTextAlignmentCenter
            inRect:layout.titleRect font:self.titleFont color:self.titleColor
       shadowColor:self.titleShadowColor shadowOffset:self.titleShadowOffset];
	[self drawText:self.message textAlignment:NSTextAlignmentCenter
            inRect:layout.messageRect font:self.messageFont color:self.messageColor
       shadowColor:self.messageShadowColor shadowOffset:self.messageShadowOffset];
}

- (void)drawText:(NSString *)text textAlignment:(NSTextAlignment)textAlignment
          inRect:(CGRect)rect font:(UIFont *)font color:(UIColor *)color
...
- (void)showOverlay:(BOOL)show {
	if (show) {
		...
		if (self.darkenBackground) {
            ...
            if (self.shouldHorizontal) {
                if (self.horizontalOrientation==UIInterfaceOrientationLandscapeLeft) {
                    overlay.transform = CGAffineTransformMakeRotation(-M_PI_2);
                }else{
                    overlay.transform = CGAffineTransformMakeRotation(M_PI_2);
                }
            }
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
        ...
        [self setTitleColor:[UIColor colorWithRed:0 green:117.0f / 255 blue:196.0f / 255 alpha:1] forState:UIControlStateNormal];
	}
	return self;
}
